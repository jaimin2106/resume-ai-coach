  ...
  return (
    <section id="templates" className="section-spacing bg-surface-sunken">
      <div className="container-tight">
        ...
        {/* Templates Grid */}
        <StaggerContainer className="grid md:grid-cols-3 gap-6 mb-20" staggerDelay={0.1}>
          {templates.map((template) => (
            <StaggerItem key={template.title}>
              <div className="group relative h-full bg-card rounded-2xl overflow-hidden border border-border/50 shadow-sm hover:shadow-elevated-lg transition-all duration-300 hover:-translate-y-1">
                {/* Preview Area */}
        ...
        <StaggerContainer className="grid md:grid-cols-3 gap-6" staggerDelay={0.1}>
          {testimonials.map((testimonial) => (
            <StaggerItem key={testimonial.name}>
              <div className="h-full bg-card rounded-2xl p-6 border border-border/50 shadow-sm">
